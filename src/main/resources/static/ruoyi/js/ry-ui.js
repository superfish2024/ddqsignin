...
(function ($) {
	$.extend({
		...
		table: {
			...
			// 回显数据字典 没有样式
			selectDictLabelNoClass: function(datas, value) {
				var actions = [];
				$.each(datas, function(index, dict) {
					if (dict.dictValue == ('' + value)) {
						var listClass = $.common.equals("default", dict.listClass) || $.common.isEmpty(dict.listClass) ? "" : "badge";
						actions.push($.common.sprintf("<span class='%s'>%s</span>", listClass, dict.dictLabel));
						return false;
					}
				});
		...
		modal: {
			...
			open: function (title, url, width, height, callback) {
				...
				layer.open({
					...
					title: title,
					content: url,
					btn: [table.options.butText?table.options.butText:'确定', '关闭'],
					// 弹层外区域关闭
					shadeClose: true,
					yes: callback,
		...
		operate: {
			...
			edit: function(id) {
				...
				var title = "修改" + table.options.modalName;
				if(table.options.editTitle){
					title = table.options.editTitle;
				}
			...
			ajaxSuccess: function (result) {
				if (result.code == web_status.SUCCESS && table.options.type == table_type.bootstrapTable) {
					...
					$.modal.alertWarning(result.msg)
				} else if (result.code == web_status.SUCCESS && (table.option ==undefined || table.option.type == undefined)) {
					$.modal.msgSuccess(result.msg);
				} else {
					$.modal.alertError(result.msg);
				}
